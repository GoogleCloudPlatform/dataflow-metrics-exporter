 ...
 * the License.
 */
package com.google.cloud.dfmetrics.utils;
...
public class MetricsCollectorUtils {
  ...
  /**
   * Fetches default Google Credentials based on GOOGLE_APPLICATION_CREDENTIALS environment value.
   *
   * @return
   */
  ...
  /**
   * Fetches credentials from Dataflow Client.
   *
   * @param dataflow Dataflow client
   * @return
   */
  public static Credentials getCredentialsFromDataflowClient(Dataflow dataflow) {
    HttpCredentialsAdapter httpCredentialsAdapter =
        (HttpCredentialsAdapter) dataflow.getRequestFactory().getInitializer();
    return httpCredentialsAdapter.getCredentials();
  }
}
